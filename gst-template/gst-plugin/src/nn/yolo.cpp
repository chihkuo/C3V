...
bool Yolo::Detect(Mat& SrcImg, Net& net, vector<Output>& output) {
	...
	int net_width = className[m_model].size() + 5;
			...
			for (int i = 0; i < grid_y; i++) {
				for (int j = 0; j < grid_x; j++) {
					...
					if (box_score >= boxThreshold) {
						cv::Mat scores(1, className[m_model].size(), CV_32FC1, pdata + 5);
						Point classIdPoint;
	...
	for (const auto& idx : nms_result) {
		output.emplace_back(Output{ classIds[idx], confidences[idx], boxes[idx] });
	}

	return output.size() > 0;
}

bool Yolo::Detect(cv::Size& input, std::vector<cv::Mat>& netOutputImg, vector<Output>& output) {
	int col = input.width;
	...
	int net_width = className[m_model].size() + 5;
	for (int stride = 0; stride < strideSize; stride++) {    //stride
		float* pdata = (float*)netOutputImg[stride].data;		//float	//*
		int grid_x = (int)(netWidth / netStride[stride]);
		int grid_y = (int)(netHeight / netStride[stride]);
		for (int anchor = 0; anchor < 3; anchor++) {	//anchors
			const float anchor_w = netAnchors[stride][anchor * 2];
			...
			for (int i = 0; i < grid_y; i++) {
				for (int j = 0; j < grid_x; j++) {
					float box_score = sigmoid_x(pdata[4]);
					if (box_score >= boxThreshold) {
						cv::Mat scores(1, className[m_model].size(), CV_32FC1, pdata + 5);	//float
						Point classIdPoint;
						double max_class_socre;
						minMaxLoc(scores, 0, &max_class_socre, 0, &classIdPoint);
						max_class_socre = sigmoid_x(max_class_socre);
...
bool Yolo::Detect(Mat& SrcImg, Net& net, vector<Output>& output) {
	...
	float ratio_h = (float)netInputImg.rows / netHeight;
	float ratio_w = (float)netInputImg.cols / netWidth;
	int net_width = className[m_mode].size() + 5;
		...
		for (int anchor = 0; anchor < 3; anchor++) {	//anchors
			...
			for (int i = 0; i < grid_y; i++) {
				for (int j = 0; j < grid_x; j++) {
					...
					if (box_score >= boxThreshold) {
						cv::Mat scores(1, className[m_mode].size(), CV_32FC1, pdata + 5);
						Point classIdPoint;
						double max_class_socre;
						minMaxLoc(scores, 0, &max_class_socre, 0, &classIdPoint);
...
void Yolo::drawPred(Mat& img, vector<Output> result, vector<Scalar> color) {
	for (int i = 0; i < result.size(); i++) {
		...
		//printf(confidence, sizeof(confidence), "%.2f", result[i].confidence);
		sprintf(confidence, "%.2f", result[i].confidence);
		string label = className[m_model][result[i].id] + ":" + confidence;
		//string label = className[m_model][result[i].id] + ":" + to_string(result[i].confidence);
